...
$actions[] = "admin_module";


$objects = array();
$objects[] = "calendar_module";
$objects[] = "all_events";
$objects[] = "own_events";
...
PRIMARY KEY ( user_id, cal_setting )
);
...
PRIMARY KEY ( `user_id`, cal_layeruser )
);
...
/* user login */
`user_id` int(11) NOT NULL,
PRIMARY KEY ( cal_group_id, `user_id` )
);
...
/* a user in the view */
`user_id` int(11) NOT NULL,
PRIMARY KEY ( cal_view_id, `user_id` )
);
...
cal_login int(11) NOT NULL,
/* user of calendar affected */
cal_user_cal int(11) NOT NULL,
/* log types:  <ul> */
/* <li>C: Created</li>  */
...
/* user login of boss */
cal_boss int(11) NOT NULL,
/* user login of assistant */
cal_assistant int(11) NOT NULL,
...
DROP TABLE IF EXISTS `calendar_entry_calendars`;
DROP TABLE IF EXISTS `calendar_entry_repeats`;
DROP TABLE IF EXISTS `calendar_entry_repeats_not`;
...
/*
 * This is the master table of calendars
*/
CREATE TABLE calendars (
/* event id */
calendar_id INT NOT NULL auto_increment,
calendar_name VARCHAR(80) NOT NULL,
calendar_admin int(11) NOT NULL,
PRIMARY KEY ( calendar_id ),
FOREIGN KEY (`calendar_admin`) REFERENCES `user_main` (`user_id`)
);

/*
 * This table is the table of calendar user permissions
*/
...
`can_reject` INT DEFAULT 0,
UNIQUE KEY `calendarToUser` (`calendar_id`, `user_id`),
...
/* cal_id is unique integer id for event */
calendar_entry_id INT NOT NULL auto_increment,
/* calendar_entry_group: the parent event id if this event is overriding an */
/* occurrence of a repeating event */
calendar_entry_group INT NULL,
/* used when an event goes past midnight into the */
/* next day, in which case an additional entry in this table */
/* will use this field to indicate the original event cal_id */
calendar_entry_ext_id INT NULL,
/* user id of user that created the event */
calendar_entry_creator int(11) NOT NULL,
/* date of event (in YYYYMMDD format) */
calendar_entry_date INT NOT NULL,
/* event time (in HHMMSS format) */
calendar_entry_time INT NULL,
/* date the event was last modified (in YYYYMMDD format) */
calendar_entry_mod_date INT,
/* time the event was last modified (in HHMMSS format) */
calendar_entry_mod_time INT,
/* duration of event in minutes */
calendar_entry_duration INT NOT NULL,
/* event priority: 1=Low, 2=Med, 3=High */
calendar_entry_priority INT DEFAULT 2,
/* 'E' = Event, 'M' = Repeating event */
calendar_entry_type CHAR(1) DEFAULT 'E',
/* 'P' = Public, */
/* 'R' = Confidential (others can see time allocated but not what it is) */
calendar_entry_access CHAR(1) DEFAULT 'P',
/* brief description of event */
calendar_entry_name VARCHAR(80) NOT NULL,
/* full description of event */
calendar_entry_description TEXT,
PRIMARY KEY ( calendar_entry_id ),
FOREIGN KEY (`calendar_entry_creator`) REFERENCES `user_main` (`user_id`)
);

/*
 * This table associates calendar events with calendars
*/
CREATE TABLE calendar_entry_calendars (
/* event id */
calendar_entry_id INT NOT NULL,
/* participant in the event */
calendar_id INT NOT NULL,
/* status of event for this calendar: <ul> */
/* <li>A=Accepted</li> */
/* <li>R=Rejected</li> */
/* <li>W=Waiting</li>    </ul>*/
calendar_entry_status  CHAR(1) DEFAULT 'A',
/* category of the event for this calendar */
calendar_entry_category INT DEFAULT NULL,
FOREIGN KEY (calendar_entry_id) REFERENCES `calendar_entry` (`calendar_entry_id`) ON DELETE CASCADE ON UPDATE CASCADE,
FOREIGN KEY (calendar_id) REFERENCES `calendars` (`calendar_id`) ON DELETE CASCADE ON UPDATE CASCADE
);
 ...
 * Defines repeating info about an event.
*/
CREATE TABLE calendar_entry_repeats (
/* event id */
calendar_entry_id INT DEFAULT 0 NOT NULL,
/* type of repeating:<ul> */
/* <li>daily - repeats daily</li> */
/* <li>monthlyByDate - repeats on same day of the month</li> */
/* <li>monthlyByDayR - repeats on same weekday of the month */
...
/* <li>weekly - repeats every week</li> */
/* <li>yearly - repeats on same date every year</li> */
calendar_entry_type VARCHAR(20),
/* end date for repeating event (in YYYYMMDD format) */
calendar_entry_end INT,
/* frequency of repeat: 1 = every, 2 = every other, 3 = every 3rd, etc. */
calendar_entry_frequency INT DEFAULT 1,
/* which days of the week does it repeat on (only applies when cal_type = 'weekly' */
calendar_entry_days CHAR(7),
PRIMARY KEY (calendar_entry_id),
FOREIGN KEY (`calendar_entry_id`) REFERENCES `calendar_entry` (`calendar_entry_id`)
);
...
* event and (at the same time) delete any exception events.
*/
CREATE TABLE calendar_entry_repeats_not (
/* event id of repeating event */
calendar_entry_id INT NOT NULL,
/* cal_date: date event should not repeat (in YYYYMMDD format) */
calendar_entry_skipdate INT NOT NULL,
PRIMARY KEY ( calendar_entry_id, calendar_entry_skipdate )
);
...
# default settings
INSERT INTO calendar_config ( cal_setting, cal_value )
VALUES ( 'application_name', 'WebCalendar' );
INSERT INTO calendar_config ( cal_setting, cal_value )
VALUES ( 'LANGUAGE', 'Browser-defined' );
INSERT INTO calendar_config ( cal_setting, cal_value )
...
/* unique category id */
calendar_cat_id INT NOT NULL,
/* user login of category owner. */
/* If this is NULL, then it is a global category */
calendar_cat_owner int(11) NOT NULL,
/* category name */
calendar_cat_name VARCHAR(80) NOT NULL,
 ...
 * Specify preferences for a user.
*/
CREATE TABLE calendar_user_pref (
/* user login */
`user_id` int(11) NOT NULL,
/* setting name */
cal_setting VARCHAR(25) NOT NULL,
