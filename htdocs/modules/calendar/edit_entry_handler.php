...
if ( empty ( $error ) && ! $can_edit ) {
  ...
  $sql = "SELECT cal_id FROM webcal_entry_user WHERE cal_id = '$id' " .
    "AND user_id = '$login' AND cal_status IN ('W','A')";
$stmt = $dbh->prepare($sql);
...
if ( $allow_conflicts != "Y" && empty ( $confirm_conflicts ) &&
  ...
  $ex_days = array ();
  if ( ! empty ( $id ) ) {
    $stmt = $dbh->prepare("SELECT cal_date FROM webcal_entry_repeats_not " .
      "WHERE cal_id = $id");
...
if ( empty ( $error ) ) {
  ...
  $stmt = $dbh->prepare("SELECT MAX(cal_id) FROM webcal_entry");
    ...
    $sql = "SELECT user_id, cal_status, cal_category FROM webcal_entry_user " .
      "WHERE cal_id = $id ";
$dbh = new DB_Mysql();
$stmt = $dbh->prepare($sql);
    if ( $stmt->execute() ) {
      for ( $i = 0; $tmprow = $stmt->fetch_row(); $i++ ) {
    ...
    if ( empty ( $error ) ) {
$dbh = new DB_Mysql();
$stmt = $dbh->prepare("DELETE FROM webcal_entry WHERE cal_id = $id");
$stmt->execute();
$stmt = $dbh->prepare("DELETE FROM webcal_entry_user WHERE cal_id = $id");
$stmt->execute();
$stmt = $dbh->prepare("DELETE FROM webcal_entry_repeats WHERE cal_id = $id");
$stmt->execute();
$stmt = $dbh->prepare("DELETE FROM webcal_site_extras WHERE cal_id = $id");
$stmt->execute();
    }
    $newevent = false;
  }
...
$e_created_by = ( ! empty ( $old_create_by ) && 
  ...
  if ( strlen ( $hour ) > 0 && $timetype != 'U' ) {
    $e_time = sprintf ( "%02d%02d00, ", $hour, $minute );
  } else {
    $e_time = "-1, ";
  }
  $e_duration = sprintf ( "%d, ", $duration );
  $e_priority = ! empty ( $priority ) ? sprintf ( "%d,", $priority ) : "2,";
  $e_access = empty ( $access ) ? "P" : "$access";
  ...
  if ( empty ( $error ) ) {
    calendar::add_entry($id, $old_id, $e_created_by, $e_date, $e_time, date ( "Ymd" ), 
                        date ( "Gis" ), $e_duration, $e_priority, $e_access, $e_caltype, $name, $description);
...
$stmt = $dbh->prepare("DELETE FROM webcal_entry_user WHERE cal_id = $id " .
      "AND user_id = '$participants[$i]'");
$stmt->execute();
    if ( empty ( $my_cat_id ) ) $my_cat_id = 'NULL';
    $sql = "INSERT INTO webcal_entry_user " .
      "( cal_id, user_id, cal_status, cal_category ) VALUES ( $id, '" .
