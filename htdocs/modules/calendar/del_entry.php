...
$stmt = $dbh->prepare("SELECT cal_create_by FROM webcal_entry WHERE cal_id = $id");
if ( $stmt->execute() ) {
  $row = $stmt->fetch_row (  );
  $owner = $row[0];
...
$event_repeats = false;
$stmt = $dbh->prepare("SELECT COUNT(cal_id) FROM webcal_entry_repeats " .
  "WHERE cal_id = $id");
...
$stmt = $dbh->prepare( "SELECT cal_date FROM webcal_entry WHERE cal_id = $id");
    if ( $stmt->execute() ) {
  ...
  if ( $is_admin || $my_event ) {
    ...
    // Waiting on approval).
    $sql = "SELECT user_id FROM webcal_entry_user WHERE cal_id = $id " .
...
if ( $stmt->execute() ) {
   ...
   translate("Notification") . ": " . $name,
    ...
    if ( $override_repeat ) {
            $stmt = $dbh->prepare( "INSERT INTO webcal_entry_repeats_not ( cal_id, cal_date ) " .
        "VALUES ( $id, $date )");
      $stmt->execute(); 
      ...
      // that were entered.
     if ( $event_repeats ) {
        $stmt = $dbh->prepare("SELECT cal_id FROM webcal_entry " .
   ...
   $ex_events = array ();
     ...
     $ex_events[] = $row[0];
          ...
          for ( $i = 0; $i < count ( $ex_events ); $i++ ) {
           $stmt = $dbh->prepare("SELECT cal_login FROM " .
         ...
         activity_log ( $ex_events[$i], $login, $delusers[$j],
                  $LOG_DELETE, "" );
               $stmt = $dbh->prepare("UPDATE webcal_entry_user SET cal_status = 'D' " .
