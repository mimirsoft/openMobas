...
<div id="trailer">
<?php
  if ( $this->login == "__public__" && $this->public_access_others != "Y" ) {
    // don't allow them to see other people's calendar
  } else if ( $this->allow_view_other == "Y" || $this->is_admin ) {
        
        $userlist = Framework_helper::get_my_users ();
        if ($this->nonuser_enabled == "Y" ) {
            $nonusers = Calendar::get_nonuser_cals ();
        ...
        if ( strstr ( $STARTVIEW, "view" ) )
            ...
            $url = $STARTVIEW;
        ?>
        <form action="<?php echo $url;?>" id="userform" method="get" name="SelectUser">
...
<?php
  ...
  for ( $i = 0; $i < 25; $i++ ) {
    ...
    echo ">";
    echo Calendar::date_to_str ( date ( "Ymd", $d ), $this->DATE_FORMAT_MY, false, true );
    echo "</option>\n";
  }
?>
</select>
...
<?php
  ...
  $wkstart = mktime ( 3, 0, 0, $m, $d - ( $wday - $this->WEEK_START ), $y );
  ...
  for ( $i = -7; $i <= 7; $i++ ) {
    ...
    echo ">";
    if ( ! empty ( $this->PULLDOWN_WEEKNUMBER ) && $this->PULLDOWN_WEEKNUMBER = "Y" ) {
      echo  "(" . week_number ( $twkstart ) . ")&nbsp;&nbsp;";
    }
    printf ( "%s - %s",
      Calendar::date_to_str ( date ( "Ymd", $twkstart ), $this->DATE_FORMAT_MD, false, true ),
      Calendar::date_to_str ( date ( "Ymd", $twkend ), $this->DATE_FORMAT_MD, false, true ) );
    echo "</option>\n";
  }
?>
</select>
...
if ( $single_user != "Y" ) {
  ...
  $goto_link[] = "<a title=\"" . 
    ...
    "href=\"$todayURL\">" . 
    translate("Today") . "</a>";
  if ( $this->login != '__public__' && $this->readonly == 'N' ) {
    $goto_link[] = "<a title=\"" . 
      translate("Admin") . "\" style=\"font-weight:bold;\" " .
      "href=\"adminhome.php\">" . 
      translate("Admin") . "</a>";
  ...
  if ( $this->login != "__public__" && $this->readonly == "N" &&
    ( $this->require_approvals == "Y" || $this->public_access == "Y" ) ) {
...
if ( ! $use_http_auth ) {
 if ( empty ( $login_return_path ) )
  ...
  echo "<span class=\"prefix\">" .
   translate("Current User") . ":</span>&nbsp;".$USER->GetUserFullName()."&nbsp;(<a title=\"" . 
...
if ( $has_boss || ! empty ( $admincals[0] ) ||
  ...
  $grouplist = Framework_helper::user_get_boss_list ( $login );
  ...
  for ( $i = 0; $i < count ( $grouplist ); $i++ ) {
    ...
    $xurl = Framework_helper::get_preferred_view ( "", "user=$l" );
