...
class Fileupload {
	...
	public function upload($filename='', $destination="") 
	{
		global $DB_SETTINGS;
                if($destination == "")
                {
                    $destination="../../../files/".$DB_SETTINGS['dbname']."/";
                }
                $this->make_error(0);
		...
		if ( $num_uploads > 1) {
			...
			for ($i=0; $i<$num_uploads; $i++) {
				...
				$this->file['mimeName']     = (isset($_FILES[$filename]['type'][$i]))     ? $_FILES[$filename]['type'][$i] : '';
			...
			$this->file['type']     = (isset($_FILES[$filename]['type']))     ? $_FILES[$filename]['type'] : '';
                        $this->file['mimeName']     = (isset($_FILES[$filename]['type']))     ? $_FILES[$filename]['type'] : '';
		...
		if (isset($file_array['tmp_name']) && $file_array['tmp_name']) {
			...
			if (!$this->check_file($file_array)) {
				...
				return FALSE;
                        ...
                        $file_array['path'] = $this->destination_dir;
                        $file_array['name'] =  $this->save_file_data($file_array);
                        if(stristr($file_array['type'], "image"))
                        {
                            $this->create_thumb($file_array, "../../thumbnails/".$DB_SETTINGS['dbname']."/", "200", "200");
                        }
                        $file_array['basename'] =  $file_array['name'];
	...
	/**
	 * File info database handler
	 * 
	 * @access private
	 */
	private function save_file_data(&$file_array='') 
	{
            $dbh = new DB_Mysql();
            $stmt = $dbh->prepare("INSERT INTO files_main 
                                            SET file_id='NULL', 
                                                fileName='$file_array[name]', 
                                                baseName='$file_array[basename]', 
                                                fileExt='$file_array[extension]', 
                                                mimeType='$file_array[type]', 
                                                mimeName='$file_array[mimeName]', 
	 ...
	 * @access private
	 */
	private function create_thumb(&$file_array='',$target_dir, $new_w,$new_h)
	{
            if($file_array['extension'] == ".jpg" || $file_array['extension'] == ".jpeg")
            {
            ...
            //find out which axis to scale, which to fix to the passed $new_
            if ($old_x > $old_y) {
                    $thumb_w=$new_w;
            ...
            if($file_array['extension'] == ".jpg" || $file_array['extension'] == ".jpeg")
            {
                imagejpeg($dst_img, $target_dir.$file_array['name']); 
	...
	private function move_file() 
	{
		...
		switch((int) $this->overwrite_mode) {
			case 1: // overwrite file with same name
				if (!@move_uploaded_file($this->file['tmp_name'], $this->file['path'] . $this->file['name'])) {
					$this->make_error(6);
					return FALSE;
				}
				break;
			case 2: // if file exists, rename upload file
				...
				while(file_exists($this->file['path'] . $base . $copy . $this->file['extension'])) {
					$copy = "_copy" . $n;
					$n++;
				}
			...
			default: // if filename exists, do nothing / flag error
				if (file_exists($this->file['path'] . $this->file['name'])) {
					$this->make_error(5);
					return FALSE;
				} else {
					if (!@move_uploaded_file($this->file['tmp_name'], $this->file['path'] . $this->file['name'])) {
